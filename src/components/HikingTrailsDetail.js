...
import {showDetails,showEvents,setVisibles,showComments,showModal} from "../Slices/placeSlice";
      ...
      <div className={placeStyle.detailcontainer}>
        <div className={placeStyle.left}>
          ...
          <div className={placeStyle['event-container']}>          
            ...
            {eventList.length<=visibles? <p>-----</p>: <button onClick={loadMore}>Load More</button>}
          </div>
        </div>
        ...
        <div className={placeStyle.right}>
            ...
            <div className={placeStyle['event-container']}>       
              ...
              {cmList.length===details.info.cmnum|| cmList.length===0? <p>-----</p>:<button onClick={()=>{loadMoreCm(cmList.length)}}>Load More</button>}
...
const Modal=()=>{
  const { modalDisplay } = useSelector((state)=>{return state.place});
  const dispatch = useDispatch();
